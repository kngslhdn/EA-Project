...
#property description "XAU APEX V8.1 - regime-adaptive multi-engine XAUUSD EA"
...
input ulong Magic=26081801;
input string CommentText="XAU APEX V8.1";
...
//---------- OPTIONAL LIVE NEWS FILTER
// Keep false for reproducible Strategy Tester runs. Enable only for live/demo after verifying broker calendar data.
...
int hEFast=-1,hESlow=-1,hRFast=-1,hRSlow=-1,hADX=-1,hATR=-1,hRSI=-1;
datetime lastBar=0,cooldownUntil=0;
double peakEquity=0.0;
int dayKey=-1,consecutiveLosses=0;

void Log(const string s){if(DebugMode)Print("XAU APEX V8.1 | ",s);}
...
bool SpreadOK(){MqlTick t;if(!SymbolInfoTick(_Symbol,t))return false;double sp=t.ask-t.bid;if(MaxSpreadPrice>0&&sp>MaxSpreadPrice){Log("BLOCK SPREAD="+DoubleToString(sp,_Digits));return false;}return true;}
bool SessionVWAP(double &vwap){datetime now=TimeCurrent();MqlDateTime d;TimeToStruct(now,d);d.hour=SessionStartHour;d.min=0;d.sec=0;datetime from=StructToTime(d);if(now<from)from-=86400;MqlRates r[];int n=CopyRates(_Symbol,EntryTF,from,now-1,r);if(n<3)return false;double pv=0,vol=0;for(int i=0;i<n;i++){double tv=(double)r[i].tick_volume;double typical=(r[i].high+r[i].low+r[i].close)/3.0;pv+=typical*tv;vol+=tv;}if(vol<=0)return false;vwap=pv/vol;return true;}

//---------- VOLATILITY STATE
bool ATRState(double &atr,double &ratio){if(!Buf(hATR,0,1,atr))return false;int nNeed=MathMax(10,ATRBaselinePeriod);double a[];int n=CopyBuffer(hATR,0,2,nNeed,a);if(n<10)return false;double sum=0;for(int i=0;i<n;i++)sum+=a[i];double base=sum/n;if(base<=0)return false;ratio=atr/base;return true;}
...
//---------- LIFECYCLE
int OnInit(){trade.SetExpertMagicNumber(Magic);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(_Symbol);hEFast=iMA(_Symbol,EntryTF,EntryFastEMA,0,MODE_EMA,PRICE_CLOSE);hESlow=iMA(_Symbol,EntryTF,EntrySlowEMA,0,MODE_EMA,PRICE_CLOSE);hRFast=iMA(_Symbol,RegimeTF,RegimeFastEMA,0,MODE_EMA,PRICE_CLOSE);hRSlow=iMA(_Symbol,RegimeTF,RegimeSlowEMA,0,MODE_EMA,PRICE_CLOSE);hADX=iADX(_Symbol,RegimeTF,RegimeADXPeriod);hATR=iATR(_Symbol,EntryTF,ATRPeriod);hRSI=iRSI(_Symbol,EntryTF,RSIPeriod,PRICE_CLOSE);if(hEFast<0||hESlow<0||hRFast<0||hRSlow<0||hADX<0||hATR<0||hRSI<0){Print("XAU APEX V8.1 INIT FAILED: indicator handle");return INIT_FAILED;}peakEquity=AccountInfoDouble(ACCOUNT_EQUITY);DailyReset();Print("XAU APEX V8.1 READY ",_Symbol);return INIT_SUCCEEDED;}
void OnDeinit(const int reason){if(hEFast>=0)IndicatorRelease(hEFast);if(hESlow>=0)IndicatorRelease(hESlow);if(hRFast>=0)IndicatorRelease(hRFast);if(hRSlow>=0)IndicatorRelease(hRSlow);if(hADX>=0)IndicatorRelease(hADX);if(hATR>=0)IndicatorRelease(hATR);if(hRSI>=0)IndicatorRelease(hRSI);}
void OnTick(){DailyReset();UpdatePeak();Manage();if(!NewBar())return;if(!SessionOK()||!SpreadOK()||RiskBlocked()||NewsBlocked())return;if(OnePositionOnly&&Positions()>0)return;Direction signal;EngineType engine;double atr,atrRatio,quality,regimeScore;if(!GetSignal(signal,engine,atr,atrRatio,quality,regimeScore))return;if(signal==BUY_SIGNAL&&!AllowBuy)return;if(signal==SELL_SIGNAL&&!AllowSell)return;Enter(signal,engine,atr,atrRatio,quality,regimeScore);}
