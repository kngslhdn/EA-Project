...
#property description "XAU APEX V9 - evidence-led trend/pullback engine for XAUUSD"
...
input ulong Magic=26081901;
input string CommentText="XAU APEX V9";
input bool AllowBuy=true;
input bool AllowSell=true;
input bool OnePositionOnly=true;

//---------- REGIME: intentionally simple; V8 over-filtered/over-traded
...
//---------- OPTIONAL BREAKOUT: OFF in baseline because V8 added too many low-quality trades
input bool EnableBreakout=false;
...
input double BreakoutRSIBuyMin=55.0;
input double BreakoutRSIBuyMax=72.0;
input double BreakoutRSISellMin=28.0;
input double BreakoutRSISellMax=45.0;
...
void Log(string s){ if(DebugMode) Print("XAU APEX V9 | ",s); }
...
bool NewBar(){ datetime t[1]; if(CopyTime(_Symbol,EntryTF,0,1,t)!=1)return false; if(t[0]==g_last_bar)return false; g_last_bar=t[0]; return true; }
int DayKey(){ MqlDateTime d; TimeToStruct(TimeCurrent(),d); return d.year*10000+d.mon*100+d.day; }
void DailyReset(){ int k=DayKey(); if(k!=g_day_key){g_day_key=k;g_consecutive_losses=0;g_cooldown_until=0;Log("NEW DAY");} }
...
void UpdatePeak(){ double e=AccountInfoDouble(ACCOUNT_EQUITY); if(e>g_peak_equity)g_peak_equity=e; }
void DayWindow(datetime &from,datetime &to){ MqlDateTime d;TimeToStruct(TimeCurrent(),d);d.hour=0;d.min=0;d.sec=0;from=StructToTime(d);to=TimeCurrent(); }
...
bool SpreadOK(){ MqlTick t;if(!SymbolInfoTick(_Symbol,t))return false;double sp=t.ask-t.bid;return MaxSpreadPrice<=0||sp<=MaxSpreadPrice; }
bool RiskBlocked(){ double bal=AccountInfoDouble(ACCOUNT_BALANCE),eq=AccountInfoDouble(ACCOUNT_EQUITY);if(bal<=0)return true;if(MaxDailyLossPercent>0&&TodayPnL()<=-bal*MaxDailyLossPercent/100.0)return true;if(MaxEquityDDPercent>0&&g_peak_equity>0&&(g_peak_equity-eq)>=g_peak_equity*MaxEquityDDPercent/100.0)return true;if(MaxTradesPerDay>0&&TradesToday()>=MaxTradesPerDay)return true;if(MaxConsecutiveLosses>0&&g_consecutive_losses>=MaxConsecutiveLosses)return true;if(g_cooldown_until>TimeCurrent())return true;return false;}

bool SessionVWAP(double &vwap){ if(!UseSessionVWAP)return true; MqlDateTime d;TimeToStruct(TimeCurrent(),d);d.hour=SessionStartHour;d.min=0;d.sec=0;datetime from=StructToTime(d);if(TimeCurrent()<from)from-=86400;MqlRates r[];int n=CopyRates(_Symbol,EntryTF,from,TimeCurrent()-1,r);if(n<3)return false;double pv=0,vol=0;for(int i=0;i<n;i++){double v=(double)r[i].tick_volume;if(v<=0)continue;pv+=((r[i].high+r[i].low+r[i].close)/3.0)*v;vol+=v;}if(vol<=0)return false;vwap=pv/vol;return true;}

bool ATRValue(double &atr){ return Buf(hATR,0,1,atr)&&atr>0; }
bool Regime(Direction &dir,double &atr,double &sep,double &adx){ double rf,rs,c,plus,minus;if(!Buf(hRFast,0,1,rf)||!Buf(hRSlow,0,1,rs)||!Buf(hADXRegime,0,1,adx)||!Buf(hADXRegime,1,1,plus)||!Buf(hADXRegime,2,1,minus))return false;if(!ATRValue(atr))return false;if(atr<ATRMin||atr>ATRMax)return false;c=iClose(_Symbol,RegimeTF,1);if(c<=0)return false;sep=MathAbs(rf-rs)/atr;dir=DIR_NONE;if(sep<MinTrendSeparationATR||adx<RegimeADXMin)return true;if(rf>rs&&c>rf&&plus>minus)dir=DIR_BUY;else if(rf<rs&&c<rf&&minus>plus)dir=DIR_SELL;return true;}

bool CandleQuality(const MqlRates &s,const double atr,const Direction dir){ double range=s.high-s.low;if(range<=0||range>atr*ExhaustionATR)return false;double body=MathAbs(s.close-s.open);if(body/range<MinBodyFraction)return false;double loc=(s.close-s.low)/range;if(dir==DIR_BUY){if(s.close<=s.open||loc<MinCloseLocation)return false;}else{if(s.close>=s.open||loc>(1.0-MinCloseLocation))return false;}return true; }

bool PullbackSignal(Direction dir,double atr,double vwap,double &score){ score=0;double ef,es,adx,rsi;if(!Buf(hEFast,0,1,ef)||!Buf(hESlow,0,1,es)||!Buf(hADXEntry,0,1,adx)||!Buf(hRSI,0,1,rsi))return false;MqlRates r[3];if(CopyRates(_Symbol,EntryTF,1,3,r)!=3)return false;MqlRates s=r[0];if(adx<EntryADXMin||!CandleQuality(s,atr,dir))return false;double dist=MathAbs(s.close-ef)/atr;if(dist>MaxChaseATR)return false;if(dir==DIR_BUY){if(!(ef>es&&s.close>ef))return false;if(rsi<RSI_Buy_Min||rsi>RSI_Buy_Max)return false;bool touch=s.low<=ef+atr*PullbackTouchATR&&s.low>=ef-atr*PullbackMaxDistanceATR;if(!touch)return false;if(UseSessionVWAP&&s.close<vwap-atr*VWAPToleranceATR)return false;double slope=ef-iMA(_Symbol,EntryTF,EntryFastEMA,0,MODE_EMA,PRICE_CLOSE);score=60.0+MathMin(20.0,MathMax(0.0,slope/atr*200.0))+MathMin(20.0,(1.0-dist/MaxChaseATR)*20.0);return score>=65.0;}else{if(!(ef<es&&s.close<ef))return false;if(rsi<RSI_Sell_Min||rsi>RSI_Sell_Max)return false;bool touch=s.high>=ef-atr*PullbackTouchATR&&s.high<=ef+atr*PullbackMaxDistanceATR;if(!touch)return false;if(UseSessionVWAP&&s.close>vwap+atr*VWAPToleranceATR)return false;double slope=ef-iMA(_Symbol,EntryTF,EntryFastEMA,0,MODE_EMA,PRICE_CLOSE);score=60.0+MathMin(20.0,MathMax(0.0,-slope/atr*200.0))+MathMin(20.0,(1.0-dist/MaxChaseATR)*20.0);return score>=65.0;} }

bool HighestLowestPrior(const int lookback,double &hh,double &ll){ MqlRates r[];int n=CopyRates(_Symbol,EntryTF,2,lookback,r);if(n<lookback)return false;hh=r[0].high;ll=r[0].low;for(int i=1;i<n;i++){hh=MathMax(hh,r[i].high);ll=MathMin(ll,r[i].low);}return true; }
bool BreakoutSignal(Direction dir,double atr,double vwap,double &score){ score=0;if(!EnableBreakout)return false;double ef,es,adx,rsi;if(!Buf(hEFast,0,1,ef)||!Buf(hESlow,0,1,es)||!Buf(hADXEntry,0,1,adx)||!Buf(hRSI,0,1,rsi))return false;MqlRates s[2];if(CopyRates(_Symbol,EntryTF,1,2,s)!=2)return false;if(!CandleQuality(s[0],atr,dir)||adx<EntryADXMin+2)return false;double hh,ll;if(!HighestLowestPrior(BreakoutLookback,hh,ll))return false;if(dir==DIR_BUY){if(s[0].close<=hh+atr*BreakoutATR||ef<=es||rsi<BreakoutRSIBuyMin||rsi>BreakoutRSIBuyMax)return false;if(UseSessionVWAP&&s[0].close<vwap-atr*VWAPToleranceATR)return false;}else{if(s[0].close>=ll-atr*BreakoutATR||ef>=es||rsi<BreakoutRSISellMin||rsi>BreakoutRSISellMax)return false;if(UseSessionVWAP&&s[0].close>vwap+atr*VWAPToleranceATR)return false;}double dist=MathAbs(s[0].close-(dir==DIR_BUY?ef:ef))/atr;if(dist>BreakoutMaxATR)return false;score=70.0;return true;}
...
double LotForRisk(ENUM_ORDER_TYPE type,double entry,double sl){double money=AccountInfoDouble(ACCOUNT_EQUITY)*RiskPercent/100.0;if(money<=0)return 0;double pnl=0;if(!OrderCalcProfit(type,_Symbol,1.0,entry,sl,pnl))return 0;double one=MathAbs(pnl);if(one<=0)return 0;return NVol(money/one);}

void Enter(Direction dir,double atr,double regimeADX,double regimeSep,double quality){ MqlTick t;if(!SymbolInfoTick(_Symbol,t))return;ENUM_ORDER_TYPE type=dir==DIR_BUY?ORDER_TYPE_BUY:ORDER_TYPE_SELL;double entry=type==ORDER_TYPE_BUY?t.ask:t.bid;double sd=atr*SL_ATR;if(sd<=0)return;double tpR=NormalTP_R;if(UseAdaptiveTP&&regimeADX>=StrongADX&&regimeSep>=StrongSeparationATR&&quality>=80.0)tpR=StrongTP_R;double sl=NPrice(type==ORDER_TYPE_BUY?entry-sd:entry+sd);double tp=NPrice(type==ORDER_TYPE_BUY?entry+sd*tpR:entry-sd*tpR);if(!StopOK(type,entry,sl))return;double lot=LotForRisk(type,entry,sl);if(lot<=0)return;trade.SetExpertMagicNumber(Magic);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(_Symbol);string c=CommentText+(dir==DIR_BUY?" PULLBACK BUY":" PULLBACK SELL");bool ok=type==ORDER_TYPE_BUY?trade.Buy(lot,_Symbol,0,sl,tp,c):trade.Sell(lot,_Symbol,0,sl,tp,c);if(ok)Log("OPEN "+(dir==DIR_BUY?"BUY":"SELL")+" lot="+DoubleToString(lot,VolDigits())+" TP_R="+DoubleToString(tpR,2));else Print("XAU APEX V9 ORDER FAIL ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription()); }

void ManagePositions(){ double atr;if(!ATRValue(atr))return;for(int i=PositionsTotal()-1;i>=0;i--){ulong ticket=PositionGetTicket(i);if(ticket==0||!PositionSelectByTicket(ticket))continue;if(PositionGetString(POSITION_SYMBOL)!=_Symbol||(ulong)PositionGetInteger(POSITION_MAGIC)!=Magic)continue;long type=PositionGetInteger(POSITION_TYPE);double open=PositionGetDouble(POSITION_PRICE_OPEN),sl=PositionGetDouble(POSITION_SL),tp=PositionGetDouble(POSITION_TP);double price=type==POSITION_TYPE_BUY?SymbolInfoDouble(_Symbol,SYMBOL_BID):SymbolInfoDouble(_Symbol,SYMBOL_ASK);double initialR=0;if(tp>0){double implied=(MathAbs(tp-open));initialR=implied/((UseAdaptiveTP&&NormalTP_R>0)?NormalTP_R:NormalTP_R);}if(initialR<=0)initialR=atr*SL_ATR;double profitDist=type==POSITION_TYPE_BUY?price-open:open-price;double R=profitDist/initialR;double newSL=sl;bool modify=false;if(R>=BE_R){double be=type==POSITION_TYPE_BUY?open+initialR*BE_Offset_R:open-initialR*BE_Offset_R;if(type==POSITION_TYPE_BUY){if(sl==0||be>sl+_Point){newSL=be;modify=true;}}else{if(sl==0||be<sl-_Point){newSL=be;modify=true;}}}if(R>=LockStart_R){double lk=type==POSITION_TYPE_BUY?open+initialR*LockProfit_R:open-initialR*LockProfit_R;if(type==POSITION_TYPE_BUY){if(lk>newSL+_Point){newSL=lk;modify=true;}}else{if(newSL==0||lk<newSL-_Point){newSL=lk;modify=true;}}}if(R>=TrailStart_R){double tr=type==POSITION_TYPE_BUY?price-atr*TrailATR:price+atr*TrailATR;if(type==POSITION_TYPE_BUY){if(tr>newSL+_Point){newSL=tr;modify=true;}}else{if(newSL==0||tr<newSL-_Point){newSL=tr;modify=true;}}}if(modify){newSL=NPrice(newSL);if((type==POSITION_TYPE_BUY&&newSL<price)||(type==POSITION_TYPE_SELL&&newSL>price)){if(!trade.PositionModify(ticket,newSL,tp))Log("MODIFY FAIL "+IntegerToString((int)trade.ResultRetcode()));}}datetime opent=(datetime)PositionGetInteger(POSITION_TIME);if(MaxBarsInTrade>0&&opent>0){int bars=iBarShift(_Symbol,EntryTF,opent,false);if(bars>=MaxBarsInTrade&&R<TimeStopMinR){trade.PositionClose(ticket);Log("TIME STOP");}}} }

int OnInit(){ trade.SetExpertMagicNumber(Magic);g_peak_equity=AccountInfoDouble(ACCOUNT_EQUITY);g_day_key=DayKey();hEFast=iMA(_Symbol,EntryTF,EntryFastEMA,0,MODE_EMA,PRICE_CLOSE);hESlow=iMA(_Symbol,EntryTF,EntrySlowEMA,0,MODE_EMA,PRICE_CLOSE);hRFast=iMA(_Symbol,RegimeTF,RegimeFastEMA,0,MODE_EMA,PRICE_CLOSE);hRSlow=iMA(_Symbol,RegimeTF,RegimeSlowEMA,0,MODE_EMA,PRICE_CLOSE);hADXEntry=iADX(_Symbol,EntryTF,ADXPeriod);hADXRegime=iADX(_Symbol,RegimeTF,RegimeADXPeriod);hATR=iATR(_Symbol,EntryTF,ATRPeriod);hRSI=iRSI(_Symbol,EntryTF,RSIPeriod,PRICE_CLOSE);if(hEFast<0||hESlow<0||hRFast<0||hRSlow<0||hADXEntry<0||hADXRegime<0||hATR<0||hRSI<0)return INIT_FAILED;return INIT_SUCCEEDED;}
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result){if(trans.type!=TRADE_TRANSACTION_DEAL_ADD)return;ulong deal=trans.deal;if(!deal)return;if(HistoryDealGetString(deal,DEAL_SYMBOL)!=_Symbol||(ulong)HistoryDealGetInteger(deal,DEAL_MAGIC)!=Magic)return;if(HistoryDealGetInteger(deal,DEAL_ENTRY)!=DEAL_ENTRY_OUT&&HistoryDealGetInteger(deal,DEAL_ENTRY)!=DEAL_ENTRY_OUT_BY)return;double p=HistoryDealGetDouble(deal,DEAL_PROFIT)+HistoryDealGetDouble(deal,DEAL_SWAP)+HistoryDealGetDouble(deal,DEAL_COMMISSION);if(p<0){g_consecutive_losses++;g_cooldown_until=TimeCurrent()+CooldownMinutes*60;}else if(p>0){g_consecutive_losses=0;g_cooldown_until=0;}}
void OnTick(){DailyReset();UpdatePeak();ManagePositions();if(!NewBar())return;if(MyPositions()>0&&OnePositionOnly)return;if(RiskBlocked()||!SessionOK()||!SpreadOK())return;double atr,sep,radx,vwap=0,quality=0;if(!Regime(*(new Direction),atr,sep,radx))return;Direction dir; if(!Regime(dir,atr,sep,radx)||dir==DIR_NONE)return;if(!SessionVWAP(vwap))return;bool sig=false;if(dir==DIR_BUY&&AllowBuy)sig=PullbackSignal(dir,atr,vwap,quality);if(dir==DIR_SELL&&AllowSell)sig=PullbackSignal(dir,atr,vwap,quality);if(!sig&&EnableBreakout){sig=BreakoutSignal(dir,atr,vwap,quality);}if(!sig)return;Enter(dir,atr,radx,sep,quality);}
