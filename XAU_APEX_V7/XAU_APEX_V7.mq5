...
#property description "XAU APEX V7 - regime quality + true pullback/breakout + asymmetric direction + initial-R management"
...
input ulong Magic=26081707;
input string CommentText="XAU APEX V7";
...
void Log(string s){if(DebugMode)Print("XAU APEX V7 | ",s);}
...
bool Regime(Direction &dir,double &atr,double &score){double rf,rs,ef,es,adx;if(!Buf(hRFast,0,1,rf)||!Buf(hRSlow,0,1,rs)||!Buf(hEFast,0,1,ef)||!Buf(hESlow,0,1,es)||!Buf(hATR,0,1,atr)||!Buf(hADX,0,1,adx))return false;if(atr<ATRMin||atr>ATRMax){Log("BLOCK ATR="+DoubleToString(atr,2));return false;}double sep=MathAbs(rf-rs)/atr;if(sep<MinTrendSeparationATR){Log("BLOCK weak regime sep="+DoubleToString(sep,2));return false;}double c=iClose(_Symbol,RegimeTF,1);if(c<=0)return false;double sepScore=MathMin(25.0,sep/0.25*25.0);double adxScore=MathMin(25.0,MathMax(0.0,(adx-10.0)/20.0*25.0));double emaScore=rf>rs?25.0:rf<rs?25.0:0.0;double priceScore=0.0;if(rf>rs&&c>rf)priceScore=25.0;if(rf<rs&&c<rf)priceScore=25.0;score=sepScore+adxScore+emaScore+priceScore;if(rf>rs&&c>rf&&adx>=BuyADXMin&&score>=BuyMinTrendScore){dir=BUY_SIGNAL;return true;}if(rf<rs&&c<rf&&adx>=SellADXMin&&score>=SellMinTrendScore){dir=SELL_SIGNAL;return true;}dir=NONE;Log("BLOCK regime score="+DoubleToString(score,1)+" ADX="+DoubleToString(adx,1));return false;}

bool VolumeConfirm(const MqlRates &r[],int count){if(count<6)return true;double avg=0;for(int i=2;i<6;i++)avg+=(double)r[i].tick_volume;avg/=4.0;if(avg<=0)return true;return (double)r[0].tick_volume>=avg*BreakoutVolumeFactor;}

Direction Signal(Direction regime,double atr,double regimeScore,SetupType &setup){setup=SETUP_NONE;MqlRates r[8];ArraySetAsSeries(r,true);if(CopyRates(_Symbol,EntryTF,1,8,r)<8){Log("BLOCK rates");return NONE;}double ef,es;if(!Buf(hEFast,0,1,ef)||!Buf(hESlow,0,1,es))return NONE;double vwap=0;if(UseSessionVWAP&&!SessionVWAP(vwap)){Log("BLOCK VWAP");return NONE;}MqlRates s=r[0];double c=s.close,o=s.open,h=s.high,l=s.low,range=h-l,body=MathAbs(c-o);if(range<=0||range>atr*ExhaustionATR){Log("BLOCK candle range");return NONE;}if(body/range<MinBodyFraction)return NONE;double closeLocation=(c-l)/range;if(regime==BUY_SIGNAL&&closeLocation<MinCloseLocation)return NONE;if(regime==SELL_SIGNAL&&closeLocation>(1.0-MinCloseLocation))return NONE;
bool trend=regime==BUY_SIGNAL?(ef>es&&c>ef):(ef<es&&c<ef);if(!trend)return NONE;
if(UseSessionVWAP&&RequireVWAPAlignment){double tol=atr*VWAPToleranceATR;if(regime==BUY_SIGNAL&&c<vwap-tol)return NONE;if(regime==SELL_SIGNAL&&c>vwap+tol)return NONE;}
if(regime==BUY_SIGNAL){
   double touch=atr*PullbackTouchATR,maxDist=atr*PullbackMaxDistanceATR;
   bool touched=l<=ef+touch && l>=ef-maxDist;
   bool rejection=c>ef && c>o && closeLocation>=MinCloseLocation;
   bool pull=touched&&rejection;
   bool brk=c>r[1].high+atr*BreakoutATR && c>ef && c>o && (c-r[1].high)<=atr*BreakoutMaxATR;
   if(EnablePullback&&pull){setup=SETUP_PULLBACK;Log("BUY TRUE PULLBACK score="+DoubleToString(regimeScore,1));return regime;}
   if(EnableBreakout&&brk&&(!RequireBreakoutVolume||VolumeConfirm(r,8))){setup=SETUP_BREAKOUT;Log("BUY BREAKOUT score="+DoubleToString(regimeScore,1));return regime;}
}else{
   double touch=atr*PullbackTouchATR,maxDist=atr*PullbackMaxDistanceATR;
   bool touched=h>=ef-touch && h<=ef+maxDist;
   bool rejection=c<ef && c<o && closeLocation<=(1.0-MinCloseLocation);
   bool pull=touched&&rejection;
   bool brk=c<r[1].low-atr*BreakoutATR && c<ef && c<o && (r[1].low-c)<=atr*BreakoutMaxATR;
   if(EnablePullback&&pull){setup=SETUP_PULLBACK;Log("SELL TRUE PULLBACK score="+DoubleToString(regimeScore,1));return regime;}
   if(EnableBreakout&&brk&&(!RequireBreakoutVolume||VolumeConfirm(r,8))){setup=SETUP_BREAKOUT;Log("SELL BREAKOUT score="+DoubleToString(regimeScore,1));return regime;}
}
return NONE;}
...
string RKey(ulong ticket){return "XAU_APEX_V7_R_"+(string)ticket;}
void SaveInitialRisk(ulong ticket,double r){if(ticket>0&&r>0)GlobalVariableSet(RKey(ticket),r);}
double LoadInitialRisk(ulong ticket){if(ticket==0)return 0;if(!GlobalVariableCheck(RKey(ticket)))return 0;return GlobalVariableGet(RKey(ticket));}
void DeleteInitialRisk(ulong ticket){if(ticket>0&&GlobalVariableCheck(RKey(ticket)))GlobalVariableDel(RKey(ticket));}

void Enter(Direction dir,double atr,double regimeScore,SetupType setup){MqlTick t;if(!SymbolInfoTick(_Symbol,t))return;ENUM_ORDER_TYPE type=dir==BUY_SIGNAL?ORDER_TYPE_BUY:ORDER_TYPE_SELL;double entry=type==ORDER_TYPE_BUY?t.ask:t.bid;double sd=atr*SL_ATR;if(sd<=0)return;double tpR=NormalTP_R;if(UseAdaptiveTP&&regimeScore>=StrongTrendScore)tpR=StrongTP_R;double sl=NPrice(type==ORDER_TYPE_BUY?entry-sd:entry+sd);double tp=NPrice(type==ORDER_TYPE_BUY?entry+sd*tpR:entry-sd*tpR);if(!StopOK(type,entry,sl)){Log("BLOCK stop");return;}double lot=LotForRisk(type,entry,sl);if(lot<=0){Log("BLOCK lot");return;}string c=CommentText+" "+(setup==SETUP_PULLBACK?"PULLBACK":"BREAKOUT");trade.SetExpertMagicNumber(Magic);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(_Symbol);bool ok=type==ORDER_TYPE_BUY?trade.Buy(lot,_Symbol,0,sl,tp,c):trade.Sell(lot,_Symbol,0,sl,tp,c);if(!ok){Print("XAU APEX V7 ORDER FAIL ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());return;}ulong ticket=trade.ResultOrder();if(ticket==0){if(PositionSelect(_Symbol))ticket=(ulong)PositionGetInteger(POSITION_TICKET);}SaveInitialRisk(ticket,sd);Log("OPEN "+(dir==BUY_SIGNAL?"BUY":"SELL")+" setup="+(setup==SETUP_PULLBACK?"PULLBACK":"BREAKOUT")+" score="+DoubleToString(regimeScore,1)+" lot="+DoubleToString(lot,VolDigits())+" R="+DoubleToString(tpR,2));}

void Manage(){if(!PositionSelect(_Symbol))return;if((ulong)PositionGetInteger(POSITION_MAGIC)!=Magic)return;ulong ticket=(ulong)PositionGetInteger(POSITION_TICKET);ENUM_POSITION_TYPE type=(ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);double open=PositionGetDouble(POSITION_PRICE_OPEN),sl=PositionGetDouble(POSITION_SL),tp=PositionGetDouble(POSITION_TP);double riskDist=LoadInitialRisk(ticket);if(riskDist<=0){if(sl>0)riskDist=MathAbs(open-sl);else return;SaveInitialRisk(ticket,riskDist);}MqlTick t;if(!SymbolInfoTick(_Symbol,t))return;double price=type==POSITION_TYPE_BUY?t.bid:t.ask;double move=type==POSITION_TYPE_BUY?price-open:open-price;if(move<=0)return;double atr;if(!Buf(hATR,0,1,atr))return;double ns=sl;bool change=false;
if(move>=riskDist*BE_R){double be=NPrice(open);if((type==POSITION_TYPE_BUY&&(sl==0||be>sl))||(type==POSITION_TYPE_SELL&&(sl==0||be<sl))){ns=be;change=true;}}
...
if(move>=riskDist*TrailStart_R){double tr=NPrice(type==POSITION_TYPE_BUY?price-atr*TrailATR:price+atr*TrailATR);if(type==POSITION_TYPE_BUY&&tr>ns&&tr<price){ns=tr;change=true;}if(type==POSITION_TYPE_SELL&&(ns==0||tr<ns)&&tr>price){ns=tr;change=true;}}
if(!change)return;double md=(double)SymbolInfoInteger(_Symbol,SYMBOL_TRADE_STOPS_LEVEL)*_Point;if(type==POSITION_TYPE_BUY)ns=MathMin(ns,NPrice(price-md));else ns=MathMax(ns,NPrice(price+md));if(sl!=0&&MathAbs(ns-sl)<_Point)return;trade.PositionModify(_Symbol,ns,tp);}

void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result){if(trans.type!=TRADE_TRANSACTION_DEAL_ADD)return;ulong deal=trans.deal;if(deal==0)return;if(!HistoryDealSelect(deal))return;if(HistoryDealGetString(deal,DEAL_SYMBOL)!=_Symbol)return;if((ulong)HistoryDealGetInteger(deal,DEAL_MAGIC)!=Magic)return;long entry=HistoryDealGetInteger(deal,DEAL_ENTRY);if(entry!=DEAL_ENTRY_OUT&&entry!=DEAL_ENTRY_OUT_BY)return;double p=HistoryDealGetDouble(deal,DEAL_PROFIT)+HistoryDealGetDouble(deal,DEAL_SWAP)+HistoryDealGetDouble(deal,DEAL_COMMISSION);if(p<0){consecutiveLosses++;cooldownUntil=TimeCurrent()+CooldownMinutes*60;Log("LOSS consecutive="+(string)consecutiveLosses);}else if(p>0){consecutiveLosses=0;Log("WIN reset consecutive losses");}if(trans.position>0)DeleteInitialRisk(trans.position);}

int OnInit(){trade.SetExpertMagicNumber(Magic);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(_Symbol);hEFast=iMA(_Symbol,EntryTF,EntryFastEMA,0,MODE_EMA,PRICE_CLOSE);hESlow=iMA(_Symbol,EntryTF,EntrySlowEMA,0,MODE_EMA,PRICE_CLOSE);hRFast=iMA(_Symbol,RegimeTF,RegimeFastEMA,0,MODE_EMA,PRICE_CLOSE);hRSlow=iMA(_Symbol,RegimeTF,RegimeSlowEMA,0,MODE_EMA,PRICE_CLOSE);hADX=iADX(_Symbol,RegimeTF,ADXPeriod);hATR=iATR(_Symbol,EntryTF,ATRPeriod);if(hEFast<0||hESlow<0||hRFast<0||hRSlow<0||hADX<0||hATR<0)return INIT_FAILED;peakEquity=AccountInfoDouble(ACCOUNT_EQUITY);DailyReset();Print("XAU APEX V7 READY ",_Symbol);return INIT_SUCCEEDED;}
